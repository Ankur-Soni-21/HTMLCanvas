...
function Particle(x, y, radius, color) {
    ...
    this.update = function (particles) {
        ...
        for (let i = 0; i < particles.length; i++) {
            ...
            if (getDistance(this.x, particles[i].x, this.y, particles[i].y) - this.radius - particles[i].radius <= 0) {
...
function init() {
    particles = [];
    for (let i = 0; i < 300; i++) {
        let radius = randomIntGenerator(5, 15);
        let x = randomIntGenerator(radius, innerWidth - radius);
        let y = randomIntGenerator(radius, innerHeight - radius);
        let color = randomColor(colors);


        if (i !== 0) {
            for (let j = 0; j < particles.length; j++) {
                if (getDistance(x, particles[j].x, y, particles[j].y) - radius - particles[j].radius < 0) {
                    x = randomIntGenerator(radius, innerWidth - radius);
                    y = randomIntGenerator(radius, innerHeight - radius);
                    //restart loop
                    j = -1;
                }
            }
